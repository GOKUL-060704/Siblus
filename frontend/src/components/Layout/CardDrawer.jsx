...
function CardDrawer({ drawerOpen, toggleDrawer }) {
    ...
    <div className={`fixed top-0 right-0 w-3/4 sm:w-1/2 md:w-120 h-full bg-white shadow-lg transform transition-transform duration-300 flex flex-col z-50 ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className='flex justify-end p-4'>
        <button onClick={toggleDrawer}>
          <IoMdClose className='h-6 w-6 text-grey-700' />
        </button>
      </div>
      ...
      {cart?.products?.length > 0 && (<div className='p-4 bg-white sticky bottom-0'>
        ...
        <p className='text-xs text-gray-500 mt-2 tracking-tighter text-center'>
